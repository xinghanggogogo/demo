...
class Handler(BaseHandler):
	...
	def index_page(self, response):
		for each in response.doc('.lady-name').items():
			self.crawl(each.attr.href, callback=self.detail_page, fetch_type='js')	#fetch_type

	def detail_page(self, response):
		domain = response.doc('.mm-p-domain-info li > span').text()	#注意路径的获取 as
		if domain:
	...
	def domain_page(self, response):
		...
		brief = response.doc('.mm-aixiu-content').text()		#简介
		imgs = response.doc('.mm-aixiu-content img').items()

		dir_path = self.deal.mkDir(name)	#这是一个目录名
